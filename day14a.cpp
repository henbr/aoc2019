...
";
using Chemical = pair<string, int>;
...
struct Factory {
    unordered_map<string, int> leftovers;
...
int get_amount(const Chemical &c) {
    return c.second;
}
...
void add_leftover(Factory &f, int amount, const string &chemical) {
...
void do_reaction(Factory &f, int amount, const string &chemical) {
    if (chemical == "ORE") {
        f.consumed += amount;
        return;
    }
    ...
    int num_reactions = amount / get_amount(r.to);
...
bool process_leftover(Factory &f, int amount, const string &chemical) {
    if (chemical == "ORE") {
        f.consumed -= amount;
    ...
    for (auto &c : r.from) {
        for (int i = 0; i < num_reactions; i++) {
            add_leftover(f, get_amount(c), get_name(c));
        }
    }
...
int main() {
    ...
    process_leftovers(f);
    cout << "Amount of ore: " << f.consumed << endl;
}
