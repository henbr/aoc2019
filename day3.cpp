...
optional<vec2> read_move(istringstream &iss) {
    string move;
    getline(iss, move, ',');
    ...
    vec2 res;
    switch(dir) {
        case 'L': res = {-len, 0}; break;
        case 'R': res = {len, 0}; break;
        case 'U': res = {0, -len}; break;
        case 'D': res = {0, len}; break;
        default: throw runtime_error("Unknown move");
    }

    return res;
}

vector<segment> read_wire(istringstream &iss) {
...
optional<vec2> test_intersection(const segment &s1, const segment &s2) {
    ...
    if (!((v->p1.y < h->p1.y && v->p2.y > h->p1.y)
        || (v->p2.y < h->p1.y && v->p1.y > h->p1.y))) {
        return nullopt;
    }

    if (!((h->p1.x < v->p1.x && h->p2.x > v->p1.x)
        || (h->p2.x < v->p1.x && h->p1.x > v->p1.x))) {
        return nullopt;
    }

    vec2 intersection = {v->p1.x, h->p1.y};
    return intersection;
}

int get_mdist(vec2 p) {
    return abs(p.x) + abs(p.y);
}

int get_seg_steps(const segment &seg, const vec2 &intersection) {
    int dx = abs(intersection.x - seg.p1.x);
    int dy = abs(intersection.y - seg.p1.y);
    return dx + dy + seg.steps;
}
...
int main() {
    ...
    vector<segment> wire1 = read_wire(iss1);
    vector<segment> wire2 = read_wire(iss2);
    int steps = 1 << 30;
    for (auto s1 : wire1) {
        for (auto s2 : wire2) {
            auto intersect = test_intersection(s1, s2);
            if (intersect) {
                int s1_steps = get_seg_steps(s1, *intersect);
                int s2_steps = get_seg_steps(s2, *intersect);
                steps = min(steps, s1_steps + s2_steps);
            }
        }
    }
