...
vector<int> reconstruct_path(RobotState &rs, vector<int> came_from) {
    int goal = get_offset(rs.tankX, rs.tankY);
    int start = get_offset(rs.x, rs.x);
...
vector<int> find_path(RobotState &rs) {
    ...
    auto cmp_less = [&](int l, int r) {
        ...
        int lxd = abs(lpos.first - rs.tankX);
        int lyd = abs(lpos.second - rs.tankY);
        int lh = lxd + lyd;

        int rxd = abs(rpos.first - rs.tankX);
        int ryd = abs(rpos.second - rs.tankY);
        int rh = rxd + ryd;

        return (scores[l] + lh) < (scores[r] + rh);
    };
    vector<int> open;
    open.push_back(get_offset(rs.x, rs.y));
    scores[get_offset(rs.x, rs.y)] = 0;
    ...
    while(!open.empty()) {
        ...
        if (goal == current) {
            return reconstruct_path(rs, came_from);
        }
...
/*
   ...
   //     if (scores[i] < 1<<30) {
        if (came_from[i] != -1) {
            auto pos = get_pos(i);
            rs.ship[pos.second][pos.first] = Location::Path;
        }
    }*/
